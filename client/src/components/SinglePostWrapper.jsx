  ...
  return (
    <StyledWrapper margin='1rem 0rem 1rem 0rem'>
      <BoxSpaced marginBottom='1rem'>
        ...
        <PersonAddOutlinedIcon color="primary" fontSize='large' onClick={() => {console.log('add friend');}} sx={{'&:hover': {cursor: 'pointer'}}}></PersonAddOutlinedIcon>
      </BoxSpaced>
      <Typography>
        {content}
      </Typography>
      {contentPictureUrl && (
        <Box component='img' src={`${process.env.REACT_APP_API_URL}/assets/${contentPictureUrl}`} alt='Post image' width='100%' borderRadius='0.25rem'/>
      )}
    </StyledWrapper>
  )
}